...
public class TagsManager {
    ...
    public static func save(tags: [String], for build: Build, team: Team, on req: Request) throws -> Future<Tags> {
        ...
        return try build.tags.query(on: req).all().flatMap() { appTags in
    ...
    public static func delete(tagId: DbIdentifier, appId: DbIdentifier, on req: Request) throws -> Future<Void> {
        return Tag.query(on: req).filter(\Tag.id == tagId).first().flatMap() { tag in
            guard let tag = tag else {
                throw ErrorsCore.HTTPError.notFound
            }
            return try req.me.teams().flatMap() { teams in
                return try Build.query(on: req).safeBuild(id: appId, teamIds: teams.ids).first().flatMap() { build in
